...
import UIKit
let pageColor:UIColor = UIColor.init(red: 135, green: 126, blue: 114, alpha: 0.5)
//图片的模式
enum contentMode {
...
class CycleView: UIView,UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    var countLabel:UILabel = {
        let lb = UILabel.init(frame: CGRect.init(x: KScreenW-100, y: 5, width: 80, height: 30))
    ...
    var titleLabel:UILabel = {
        let lb = UILabel.init(frame: CGRect.init(x: 0, y: 194-30, width: KScreenW, height: 30))
        lb.textColor = UIColor.white
        lb.backgroundColor = UIColor.init(white: 12, alpha: 0.3)
        lb.textAlignment = .center
    ...
    var mode : contentMode? = .scaleAspectFit
...
extension CycleView {
    ...
    internal func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        countLabel.text = "\(page)" + "/"+"\(imageURLStringArr!.count)"
